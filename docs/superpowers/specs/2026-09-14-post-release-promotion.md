...
- `PromotionCampaign.seriesItems` 是新的主工作模型：每项独立保存计划元数据、X 文案、所选 Store Product 与商店链接、视觉路径、参考截图、最终附件、图片提示词、修订和发布状态。多平台产品默认使用 macOS 主平台链接，用户可以逐条改选 iPhone / iOS 链接。
- `promotion:generateSeriesPlan` 创建 1–5 条精简计划，`promotion:generateSeriesItem` 按需生成指定条目的最终内容；每条 X 主帖必须包含当前产品对应的 App Store 链接。
- 截图和图片提示词是可选增强，不再阻塞纯文字 X 帖子的生成与发布。文案生成不生成图片提示词；用户完成文案后逐条选择“无图 / 使用截图 / 制作推广图”。
- 参考截图只作为图片制作输入，最终附件才作为发布事实；两者不得共用一个字段。每条 X 帖子按顺序记录最多 4 张最终图片，截图包装最多使用 2 张参考截图。
...
首版不在 Appilot 内生图，而是按帖子、按需输出一份与供应商无关、可以复制到其他 AI 生图平台的结构化提示词。生成 X 文案时不提前生成图片提示词。

单条帖子的视觉路径是互斥的：

```text
无图发布
使用截图 → 直接选择并排序 1–4 张最终附件
制作推广图
  ├─ 独立场景图 → 生成提示词 → 导入 1–4 张最终成品
  └─ 包装真实截图 → 选择 1–2 张参考截图 → 生成提示词 → 导入 1–4 张最终成品
```

参考截图是生成输入，不自动成为发布附件；外部生成结果在当前帖子内导入，并自动成为该帖子的最终附件。
