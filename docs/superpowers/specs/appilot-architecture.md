...
│  └──────────────────────────────────────────────┘   │
│  ┌────────────┐ ┌────────────────────────────────┐   │
│  │   Router   │ │  Theme (Tailwind CSS dark: 前缀)│   │
│  │(React Router│ │                                │   │
│  │ Hash mode) │ │                                │   │
│  └────────────┘ └────────────────────────────────┘   │
├─────────────────────────────────────────────────────┤
│  Electron Main Process (Phase 0 最小使用)            │
│  ┌───────────────┐ ┌────────────────────────────┐   │
│  │ BrowserWindow │ │ electron-store (AI Key存储) │   │
│  └───────────────┘ └────────────────────────────┘   │
├─────────────────────────────────────────────────────┤
│         Core Engine (纯 TypeScript，零 Electron 依赖) │
│  ┌───────────────┐ ┌──────────────┐ ┌────────────┐  │
│  │ Repo Analyzer │ │  AI Engine   │ │  Content   │  │
│  │ (simple-git + │ │ (openai SDK) │ │   Store    │  │
│  │  octokit)     │ │ (摘要+推文)  │ │            │  │
│  └───────────────┘ └──────────────┘ └────────────┘  │
│  ┌──────────────────────────────────────────────┐   │
│  │      Analytics Engine (手动填报 + 趋势查询)   │   │
│  └──────────────────────────────────────────────┘   │
├─────────────────────────────────────────────────────┤
│                  Data Layer                          │
│  ┌─────────────────┐                                │
...
│  │(React Router│ │    i18n    │ │  System Tray  │   │
│  │ Hash mode) │ │ (react-i18n)│ │ (Electron API)│   │
│  └────────────┘ └────────────┘ └───────────────┘   │
├─────────────────────────────────────────────────────┤
│                    Core Engine                       │
│  ┌──────────────────────────────────────────────┐   │
...
- **Presentation 不直接访问 Data** — 所有数据通过 Application 层的 State Management（Zustand）获取
...
- **Engine 包零 Electron/React 依赖** — `engine/` 是纯 TypeScript 包，可被桌面（Electron）、云端（Node.js）、移动端 PWA（浏览器）复用

### 3.3 技术栈

| 层 | 技术选型 | 理由 |
|----|----------|------|
| 桌面框架 | **Electron** (main + renderer) | macOS/Windows 桌面应用。OAuth（BrowserWindow）、系统托盘（Tray API）、自动更新（electron-updater）均为原生一等公民 API |
| UI 框架 | **React 18+ + TypeScript** | 声明式 UI，生态最大，组件库丰富 |
| UI 组件库 | **shadcn/ui + Tailwind CSS** | 组件源码自有（可任意修改），暗色模式一个 `dark:` 前缀，设计语言参考 Apple/Google 规范 |
| 状态管理 | **Zustand** | 轻量、TypeScript 原生、API 极简（一个 `create()` 函数），适合 solo 项目 |
| 路由 | **React Router** (Hash Router) | Electron 使用 `file://` 协议加载，Hash Router 不依赖服务端路由 |
| 本地数据库 | **better-sqlite3 + drizzle-orm** | better-sqlite3：同步 API、极快、Electron 主进程原生运行。drizzle-orm：类型安全的 SQL 查询构建器，migration 自动生成 |
| 凭据存储 | **electron-store + safeStorage** | Electron 原生加密存储（macOS Keychain / Windows DPAPI），API Key 和 OAuth Token 加密落盘 |
| Git 操作 | **simple-git** | git CLI 的 Promise 包装，支持所有 git 命令。Phase 0 用于读取 GitHub 仓库本地 clone 后的 git log |
| GitHub API | **@octokit/rest** | GitHub 官方维护的 REST API SDK，类型安全 |
| HTTP 客户端 | **fetch** (Node.js 18+ 原生) + **ofetch** | Node.js 18+ 原生支持 fetch；ofetch 提供更友好的错误处理 |
| AI SDK | **openai** (npm) | OpenAI 官方 SDK，兼容所有 OpenAI 兼容 API（含 DeepSeek/Groq/Ollama） |
| OAuth 流程 | **Electron BrowserWindow** | 直接打开 OAuth 授权页 → 监听 redirect URI → 自动解析 code，零自建 HTTP server |
| 图表 | **Recharts** 或 **ECharts** | React 原生图表库，趋势折线图、柱状图开箱即用 |
| 通知 | **Electron Notification API** | 系统原生通知（macOS 通知中心 / Windows Action Center） |
| i18n | **react-i18next** | React 生态最成熟的国际化方案（Phase 1+） |
...
interface AnalyticsEngine {
  // ── Phase 0 实现 ──
  /** 用户在手动填报表单中提交的统计写入（source = manual） */
  submitManualStats(entry: ManualStatsEntry): Promise<void>;
  /** 按帖子查询历史快照，供趋势折线图使用 */
  watchTrend(postId: string): AsyncIterable<PostAnalyticsSnapshot[]>;

  // ── Phase 1+ 扩展 ──
  recordApiStats(postId: string, stats: InteractionStats): Promise<void>;
  recordUrlBackfillStats(postId: string, stats: InteractionStats): Promise<void>;
  extractFromPublicUrl(url: string, platformId: string): Promise<PostStatsFromUrl>;
}
```

**Phase 0 设计说明**：仅实现 `submitManualStats` 和 `watchTrend`。`post_analytics.source` 在 Phase 0 始终为 `manual`。手动统计不经过 Plugin 接口——UI 层直接调用 `AnalyticsEngine.submitManualStats()` 写入数据库。
...
interface RepoAnalyzer {
   ...
   *  使用 @octokit/rest 读取 README/文件树/最近提交；simple-git 用于本地 clone 后的 git 操作。
   *  公开 API 限制 60 req/h（未认证），超出后提示用户提供 PAT（5,000 req/h）或降级使用缓存。 */
  connectGitHubPublicRepo(githubUrl: string): Promise<RepoIndex>;
  refresh(projectId: string): Promise<RepoIndex>;
  /** 获取仓库结构化摘要：README、技术栈（从 package.json 等提取）、目录结构、最近 10 次提交 */
  summarize(projectId: string, maxFiles?: number): Promise<RepoSummary>;
  extractHighlights(projectId: string, aiContext: AIContext): Promise<FeatureHighlight[]>;

  // ── Phase 1+ 扩展 ──
  connectLocalRepo(localPath: string): Promise<RepoIndex>;
  connectGitHubPrivateRepo(githubUrl: string, pat: string): Promise<RepoIndex>;
  detectDistributionPlatforms(projectId: string): Promise<DistributionPlatform[]>;
  detectProjectLinks(projectId: string): Promise<ProjectLinks>;
  checkReadmeCompleteness(projectId: string): Promise<ReadmeCompleteness>;
  getRecentChanges(projectId: string, since?: Date): Promise<ChangelogSummary>;
}
...
interface PlatformPlugin {
  ...
  // ── 能力声明 ──
  readonly supportedContentTypes: ContentType[];  // 文本、图片、视频、链接
  readonly rateLimits: Record<string, number>;    // 各操作的频率限制
  readonly publishMode: PublishMode;              // 编译期已知的平台发布能力上限
  readonly trackingMode: TrackingMode;            // 编译期已知的平台追踪能力上限
   ...
   */
  currentPublishMode(): Promise<PublishMode>;
  /** 运行时动态计算当前追踪模式 */
  currentTrackingMode(): Promise<TrackingMode>;

  // ── 发布 ──
  publish(content: PostContent): Promise<PostResult>;
  /** 当 publishMode == webIntent 时使用：返回预填充内容的平台 URL */
  buildWebIntentUrl?(content: PostContent): string | null;

  // ── 互动追踪 ──
  fetchNewInteractions(postId: string, since?: Date): Promise<Interaction[]>;
  getStats(postId: string): Promise<InteractionStats>;

  // ── Tier 0 URL 回填追踪 ──
  parsePublicPostUrl?(url: string): Promise<PostStatsFromUrl | null>;
...
```typescript
// 凭据的抽象基类型。各平台插件根据需要扩展。
interface Credential {
  readonly type: string;              // 凭据类型标识（"oauth_token" | "api_key" | "bot_token" | "webhook_url"）
  toJSON(): Record<string, unknown>;  // 序列化为 JSON，供 Keychain 存储
}

interface OAuthToken extends Credential {
...
interface ApiKey extends Credential {
...
interface PostContent {
  ...
  platformOptions: Record<string, unknown>; // 平台特有参数（如 tags、flairs）
}
...
interface PostResult {
  ...
  success: boolean;
  errorMessage?: string;
  requiresManualStep: boolean;    // webIntent / manualOnly 模式时为 true
  manualStepUrl?: string;         // requiresManualStep 为 true 时提供引导 URL
}

interface Interaction {
  platformId: string;
  postId: string;
  type: InteractionType;          // "comment" | "like" | "share" | "retweet" 等
  authorName: string;
  body: string;
  createdAt: Date;
  isReplied: boolean;
...
enum PublishMode { apiAuto = "api_auto", webIntent = "web_intent", manualOnly = "manual_only" }

enum TrackingMode {
  apiAuto = "api_auto",
  apiReadOnly = "api_read_only",
  urlBackfill = "url_backfill",
  manualEntry = "manual_entry",
  unavailable = "unavailable",
}

/** Tier 0：从公开 URL 提取到的帖子统计（无需 API Key） */
interface PostStatsFromUrl {
  url: string;
  viewCount?: number;
  likeCount?: number;
  commentCount?: number;
  shareCount?: number;
  recentComments?: Interaction[];
  extractedAt: Date;
  extractionMethod: "json_endpoint" | "oembed" | "headless_browser" | "html_parse";
}

/** 手动模式下用户自行填报的统计快照 */
interface ManualStatsEntry {
  postId: string;
  viewCount?: number;
  likeCount?: number;
  commentCount?: number;
...
> ⚠️ **设计说明**：此处的"插件"是架构层面的抽象（分离关注点、定义接口契约），而非用户可选的"安装/卸载"模块。所有平台插件在编译期随应用一起打包为单一可执行文件。

- **内置插件** — 4 个官方平台插件通过 npm workspace 引用，编译期全部集成。未配置凭据的插件处于"未激活"状态（`currentPublishMode()` = `manualOnly`），UI 上灰显或隐藏，不产生运行时开销
- **社区插件（Phase 4+）** — 对于社区贡献的新平台插件，MVP 后有两种可行路径：
  - **编译期集成**：社区贡献者提 PR，插件随应用一同编译发布。稳定可靠，但更新慢。
  - **进程隔离 + 标准化协议**（Phase 4 评估）：插件作为独立 Node.js 进程，通过 stdin/stdout JSON-RPC 与 Engine 通信。可行但复杂度高，需在 Phase 4 做技术验证后决定。
...
class RepoContextBuilder {
  constructor(private repoAnalyzer: RepoAnalyzer) {}

  /** Phase 0：单仓库上下文组装（GitHub 公开仓库 → Twitter 推文生成） */
  async buildContext(projectId: string): Promise<AIContext> {
    const repo = await this.repoAnalyzer.summarize(projectId);
    const highlights = await this.repoAnalyzer.extractHighlights(projectId, /* aiContext */);

    return {
      systemPrompt: [
        `你是 Appilot 的 AI 推广助手。以下是你要推广的软件项目的真实信息。`,
        ``,
        `## 产品信息`,
        `- 名称: ${repo.projectName}`,
        ...
        ``,
        `## 最近更新`,
        ...repo.recentCommits.map(c => `- ${c.date}: ${c.message}`),
        ``,
        `## 目标平台`,
        ...
        `- 每次推广内容应有所不同，避免重复`,
        `- 推广文案中自然融入项目链接`,
        ...(highlights.filter(h => h.isFocused).length > 0
          ? ["", "## 本次推广重点特性", ...highlights.filter(h => h.isFocused).map(h => `- ${h.title}`)]
          : []),
      ].join("\n"),
      repoSnapshot: repo,
      highlights,
...
> **Phase 0 简化要点**：仅组装单仓库的 README + 技术栈 + 最近提交 + 特性列表。不包含多仓库聚合（Phase 1+ 扩展）。

用户无需手动填写产品描述、关键特性或品牌语气——AI 从代码仓库中自动提取。用户可在设置中补充偏好（如"强调开源/免费特性"），这些会合并到 System Prompt 中。
...
class AIEngine {
  constructor(
    ...
    private contextBuilder: RepoContextBuilder,
    private provider: AIProvider,
  ) {}

  // ── Phase 0 实现 ──
  /** 分析仓库后生成产品摘要（名称、一句话描述、技术栈、关键特性、受众定位） */
  analyzeProduct(projectId: string): Promise<ProductSummary>;

  /** 为 Twitter 平台生成推广文案（自动适配 280 字符限制、hashtags、链接） */
  generateTweet(params: {
    projectId: string;
    stage: PromotionStage;
    customPrompt?: string;           // 用户额外指令
    focusFeatures?: FeatureHighlight[];
  }): Promise<GeneratedPost>;

  // ── Phase 1+ 扩展 ──
  generatePromotionPlan(projectId: string, opts?: {
    preferredPlatforms?: PlatformTarget[];
    focusArea?: string;
  ...
  generateCrossPlatformPosts(params: {
    projectId: string;
    platforms: PlatformTarget[];
    stage: PromotionStage;
  }): Promise<Map<PlatformTarget, GeneratedPost>>;
  ...
  generateReadmePRs(projectId: string): Promise<PRSuggestion[]>;
  generateDiffContent(suggestionId: string): Promise<string>;
}
```
...
interface TokenUsage {
  ...
  totalTokens: number;
  estimatedCost: number;       // 基于当前模型定价估算（USD）。本地模型 = 0
}

// OpenAI 兼容 Provider — 支持 OpenAI、DeepSeek、Groq、Ollama（本地）
class OpenAICompatibleProvider implements AIProvider {
  constructor(baseUrl: string, apiKey: string, model: string) {}
  // API Key 在 Phase 0 明文存 electron-store，Phase 1 迁移到 safeStorage 加密
}
```
...
**Phase 0 Schema 设计要点：**
- `projects.repo_url` — Phase 0 仅支持单 GitHub 公开仓库 URL
- `projects.repo_source` — Phase 0 固定为 `github_public`
...
- `posts.publish_mode` — Phase 0 固定为 `web_intent`
- `post_analytics.source` — Phase 0 固定为 `manual`
- 数据库实现：**better-sqlite3**（同步 API，Electron 主进程运行）+ **drizzle-orm**（类型安全 SQL 查询构建器，migration 自动生成）
- 凭据管理：`ai_config.api_key` 在 Phase 0 通过 **electron-store** 持久化（明文 JSON 文件存储），Phase 1 迁移到 **safeStorage** 加密
