...
> 本文档定义 AppPost 的**产品边界**：定位与愿景、MVP 范围（支持平台与核心功能）、开放问题、以及手动发布与分级追踪能力模型。
...
AppPost 是 OPC（一人公司）的 **AI 驱动运营中心**。它连接应用的代码仓库、发布渠道和社交媒体账号，在一个统一的界面中呈现开发进度、推广状态、下载数据和用户反馈，让独立开发者无需在 GitHub、App Store Connect、Twitter、Reddit 等多个平台之间来回切换，就能全局掌握自己所有产品的情况。

**一句话**：AppPost 让你在一屏之内看到——"我的应用开发到哪里了？推广效果怎么样？用户怎么说？花了多少钱？赚了多少钱？"

**发展路径：** 自用 → 开源社区驱动 → 商业化（买断/订阅）

### 1.2 解决的问题

独立开发者（OPC）的最大困境不是不会写代码，而是**运营碎片化**：代码在 GitHub 上、下载数据在 App Store Connect 里、用户反馈散落在 Twitter/Reddit/Discord 的评论区、推广帖子发了但不知道效果如何、花了多少 AI API 费也没算过。AppPost 把这些分散的信息汇聚到一个运营中心里，通过 AI 自动关联和提炼，让一个人也能像有运营团队一样掌握全局。
...
**结论**：无 AI 时 AppPost 是一个"跨平台发布清单 + 互动追踪工作台"——统一了发布和追踪流程，有一定实用价值。但核心差异化（代码→文案、代码→推广计划、代码→回复建议）全部依赖 AI。**对于自用验证阶段，建议始终使用 AI（本地 Ollama 零成本），不推荐无 AI 模式。** 首次设置向导中 AI API 配置步骤允许"跳过，稍后配置"，但应用会在多处提示"开启 AI 后可解锁核心功能"。
