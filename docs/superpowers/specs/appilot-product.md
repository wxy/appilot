...
> 本文档定义 Appilot 的**产品边界**：定位与愿景、MVP 范围（支持平台与核心功能）、开放问题、以及手动发布与分级追踪能力模型。
...
**结论**：无 AI 时 Appilot 是一个"跨平台发布清单 + 互动追踪工作台"——统一了发布和追踪流程，有一定实用价值。但核心差异化（代码→文案、代码→推广计划、代码→回复建议）全部依赖 AI。**对于自用验证阶段，建议始终使用 AI（本地 Ollama 零成本），不推荐无 AI 模式。** 首次设置向导中 AI API 配置步骤允许"跳过，稍后配置"，但应用会在多处提示"开启 AI 后可解锁核心功能"。
...
| 功能模块 | MVP 范围 | 后续版本 |
|----------|----------|----------|
| 项目配置 | 添加应用项目（名称、发布 URL、发布渠道 App Store/Google Play/独立分发、代码仓库路径或 GitHub URL）；支持本地仓库和 GitHub 仓库（私有仓库需 PAT）；AI 自动读取仓库内容并识别发布渠道和项目链接 | 多项目全局管理、GitLab/Bitbucket 支持 |
| **运营总览** | **单产品仪表盘**：开发进度（最近 Release、提交活跃度）、推广进度（已发布帖子数、推广阶段）、下载量（手动登记或 App Store Connect API 自动拉取）、互动汇总（跨平台评论/点赞趋势）、AI 费用统计 | 多产品宏观仪表盘、投入产出分析、收益追踪 |
| AI 推广引擎 | **代码→文案**：AI 读取仓库后自动提取关键特性、最近更新、技术亮点，生成多平台适配的推广文案草稿；**推广计划**：AI 根据 Release 节奏建议发布阶段与目标平台；**回复辅助**：AI 根据仓库上下文生成个性化互动回复建议 | 全自动代理模式、发布策略优化、多模态素材生成 |
...
| 基础统计 | 浏览量、点赞、评论数：具备追踪能力的平台自动拉取，Tier 0 回填 URL 自动提取，不可用时降级为手动填报；下载量手动登记 | 趋势分析、对比报告、下载量自动拉取 |
