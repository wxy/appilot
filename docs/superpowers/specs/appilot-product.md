...
2. **发布工作台**：以 **仓库根目录 `RELEASE_DRAFT.md` 是否存在及其文件修改时间** 为主信号。用户进入发布工作台手动检查；发现文件后，由用户确认，AI 才生成可直接粘贴到 App Store 的商店内容。App Store 元数据按编辑时机分成两类：**Promotional Text（现在可改，未来接 API 后自动更新；未接 API 时生成文本供手动更新）** 和 **需要随新版本提交并审核的字段（名称/副标题/描述/关键词/What's New）**。仓库侧完成正式发布后，自己删除 `RELEASE_DRAFT.md`；Appilot 只读文件，不负责正式发布或删除。截图文案与商店文案同属对应版本，但各自拥有独立的创建、目标语言、确认、翻译和完成状态，任何一方缺失或未完成都不阻塞另一方。截图母本固定使用界面语言；截图类型无需排序，标题和描述使用可双击复制的单行字段。生成母本不等于确定母本，用户明确确定后才开放其他语言翻译。AI 以项目档案和本次发布信息为事实依据生成整组截图母本。每个截图类型可选择一张母本语言截图，其他语言默认继承并可选择自己的本地化截图。界面按当前语言显示多列竖版卡片，每张卡片依次包含标题、描述和可点击选择图片的截图预览。整批截图文案确定后，用户选择包含 `appilot.screenshot.v1` 母板及 `appilot.title`、`appilot.description`、`appilot.image` 三个约定占位符的 Keynote 模板；Appilot 可保留可编辑 Keynote 副本，也可经 Keynote 直接导出按“序号-语言-截图类型”命名的 PNG。填充时限制文本框最大宽度以稳定换行，模板原件始终不被修改。每次发布还会提出对现有跟踪关键词的补充/删除建议。
