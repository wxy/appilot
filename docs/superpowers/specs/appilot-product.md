...
2. **发布工作台**：以 **仓库根目录 `RELEASE_DRAFT.md` 是否存在及其文件修改时间** 为主信号。用户进入发布工作台手动检查；发现文件后，由用户确认，AI 才生成可直接粘贴到 App Store 的商店内容。App Store 元数据按编辑时机分成两类：**Promotional Text（现在可改，未来接 API 后自动更新；未接 API 时生成文本供手动更新）** 和 **需要随新版本提交并审核的字段（名称/副标题/描述/关键词/What's New）**。仓库侧完成正式发布后，自己删除 `RELEASE_DRAFT.md`；Appilot 只读文件，不负责正式发布或删除。截图文案与商店文案同属对应版本，但各自拥有独立的创建、目标语言、确认、翻译和完成状态，任何一方缺失或未完成都不阻塞另一方。截图母本固定使用界面语言；截图类型无需排序，标题和描述使用可双击复制的单行字段。截图文案的语言标签按本地化名称音序排列，并与内容区组成切换卡片。流程依次为生成母本、确定母本、翻译其他语言、确定整批；母本确定后锁定母本，整批确定后锁定全部文案，通过“修改截图文案”明确重新进入编辑。AI 以项目档案和本次发布信息为事实依据生成整组截图母本。每个截图类型可选择一张母本语言截图，其他语言默认继承并可选择自己的本地化截图。界面按当前语言每行最多显示四张竖版卡片，每张卡片依次包含标题、描述和可点击选择图片的截图，占位符名称采用专用颜色。选择 Keynote 模板时立即验证 `appilot.screenshot.v1` 母板及两个文本、一个图片占位符；验证后可以在任意确认状态下一次生成填充后的 Keynote 和按“序号-语言-截图类型”命名的 PNG。Keynote 与模板同目录，PNG 位于同目录下的新子目录。填充时限制文本框最大宽度以稳定换行，模板原件始终不被修改。每次发布还会提出对现有跟踪关键词的补充/删除建议。
