...
竞品发布新版本、单条反馈等弱信号只作为 AI 上下文，不直接标成缺陷。AI 优先处理高优先级问题；如果确定性检测为空，应明确给出的是优化机会，而非假装发现问题。检测结果作为内部决策依据，不在简报里重复展示总览页已有的指标和状态。
...
| ready | 只展示按价值排序的行动建议；建议可追问、采纳或忽略，已处理条目不再显示 |
...
简报不承担数据看板职责：关键词数量、排名分布、发布进度、仓库活动、评价和竞品概况继续由下方对应卡片展示。AI 只补充跨数据判断、优先级和下一步动作；同一动作与目标的重复建议在解析阶段合并。
...
- 成本控制：简报生成 1 次调用，输出上限 2400 token；每次追问 1 次调用，追问输出上限 1200 token；均使用 `thinking` disabled；
