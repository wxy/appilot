...
- 教训 C 已完成：iTunes Search 403/429 共享熔断、冷却和调度限流已落地。

## 3. 富数据后台任务——✅ 应用运行时恢复（2026-09-10）
- `rank` / `github-sync` 继续由 daemon 无头执行。
- `ops-sync` / `reviews-sync` / `build-status` 依赖 Electron `safeStorage` 解密或完整项目富数据，
  由 Electron 运行时的专属心跳自动补跑；任务 `kind/instance` 已正确镜像到共享 DB。
- 后续可选优化：设计不暴露明文凭据的 daemon 凭据代理，再把上述三类完全迁到无头执行。

## 4. DSH 会话「活动」视图 —— ⚠️ 已按架构收敛撤销（2026-09-04）
...
## 5. DSH 主面板全局任务中心 —— ⚠️ 已按架构收敛撤销（2026-09-04）
...
- 2026-09-10 审计：移除每 10 秒把 rank 快照反灌 app_kv 的无效循环；排名页已直接读取共享 DB。
