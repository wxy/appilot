...
启动 HTTP 200，`/plugins/@appilot/dsh/client.js` 正常服务。
...
或 `disabled: true`。**卸载**：删 patch 行 + `dsh plugin --profile web remove @appilot/dsh`。
...
> ⚠️ 已废弃的做法：往 `dsh.profile.bundles` 追加 `@appilot/dsh`（会把插件变成
