...
import { inferAppVersion } from "@appilot/core/store-submission";
...
import type { GitHubRepoCapabilities } from "@appilot/core/github-api";
...
export function registerReleaseHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (_event, projectId: string, productId: string, releaseTag: string) => {
      ...
      const { checkForRelease } = await import("@appilot/core/release-watcher");
      const { readFullReadme, readRepoDescription } = await import("@appilot/core/app-store-discovery");
      const token = resolveEffectiveCredentials(s, project.id).githubToken;
      const githubReleases = await githubReleaseCandidates(
        project,
        token,
    ...
    if (release.draft) {
      if (force) {
        ...
        if (Array.isArray(includeShas) && release.material) {
          const { filterMaterial, materialToBody } = await import("@appilot/core/release-watcher");
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const { translateStoreSubmissionContent } = await import("@appilot/core/ai/release-reviewer");
  ...
  ipcMain.handle(
    ...
    async (_event, projectId: string, productId: string, releaseTag: string) => {
      ...
      const { buildStoreRebuildDraft } = await import("@appilot/core/store-submission");
