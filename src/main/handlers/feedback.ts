...
export function registerFeedbackHandlers(): void {
  ...
  ipcMain.handle("feedback:cluster", async (_event, projectId: string, productId: string) => {
    ...
    const { generateReviewThemes } = await import("@appilot/core/ai/review-insights");
    const description = (await import("@appilot/core/app-store-discovery")).readRepoDescription(project.localPath);
