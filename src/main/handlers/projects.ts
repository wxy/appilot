...
} from "@appilot-labs/appilot-core/pre-release";
import {
...
export function registerProjectsHandlers(): void {
  ipcMain.handle("projects:list", async () => {
    ...
    for (const project of cleaned) {
      ...
      if (!stale) continue;
      try {
        const { collectRepoInfo } = await import("@appilot-labs/appilot-core/git-info");
  ...
  ipcMain.handle("projects:generateKeywords", async (_event, productId: string, language: string) => {
    ...
    const { generateKeywords } = await import("@appilot-labs/appilot-core/ai/keyword-suggester");
  ...
  ipcMain.handle("projects:extractSubmissionCandidates", async (_event, productId: string, language: string, operationId = "") => {
    ...
    const provider = await createAiProvider(s);
    const { extractSubmissionCandidates } = await import("@appilot-labs/appilot-core/ai/keyword-suggester");
  ...
  ipcMain.handle("projects:pendingPauseList", async (_event, projectId: string) => {
    ...
    if (!project) return [];
    const { readFullReadme } = await import("@appilot-labs/appilot-core/app-store-discovery");
    let readme = "";
    try {
  ...
  ipcMain.handle("projects:collectRanks", async (event, productId: string, language?: string, storefront?: string) => {
    ...
    const { lookupApp } = await import("@appilot-labs/appilot-core/app-store-discovery");
